...
let data,activeDay=1,role=localStorage.tripRole||'family',notionItems=[];
...
function candidates(){try{return [...notionAttractions(activeDay),...JSON.parse(localStorage.tripCandidates||'[]').filter(x=>Number(x.day)===activeDay)]}catch{return notionAttractions(activeDay)}}
function setCandidates(v){localStorage.tripCandidates=JSON.stringify(v)}
function localCandidates(){try{return JSON.parse(localStorage.tripCandidates||'[]')}catch{return[]}}
...
async function load(){
 ...
 applyNotionToTrip();applyRole();render();$$('[data-icon]').forEach(e=>e.innerHTML=icons[e.dataset.icon])
}
...
function mergedMeals(d){
 ...
 restaurants().filter(x=>Number(x.day)===activeDay).forEach(x=>out[x.meal].push(x));return out
}
